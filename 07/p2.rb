...
def get_subs(hand)
  config = hand.split('').group_by { |e| e }.transform_values(&:size)
  return [hand] if config['J'].nil?

  config.keys.map { |card| hand.gsub('J', card) }
end
...
def parse_line(line)
  ...
  {
    hand: hand,
    type_strength: get_subs(hand).map { |t| get_type(t) }.max,
    hand_strength: calc_hand_strength(hand) || -1,
    bid: bid_str.to_i
  }
end
